...
export default function AboutPage() {
  ...
  const sectionPadTop = isMobile ? "52px" : "90px";
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#030d10", fontFamily: "'DM Sans',sans-serif" }}>
      ...
      <section style={{
        ...
        padding: isMobile ? "56px 20px 52px" : "80px 40px 72px",
        textAlign: "center", borderBottom: "1px solid rgba(0,255,209,0.07)",
      }}>
        ...
        <div style={{ position:"relative", zIndex:1 }}>
          ...
          <h1 className="_abB" style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:900, fontSize:"clamp(2.4rem,8vw,4.5rem)", textTransform:"uppercase", color:"#00ffd1", margin:"0 0 16px", lineHeight:1, textShadow:"0 0 40px rgba(0,255,209,0.3)" }}>About Me</h1>
      ...
      <section style={{
        ...
        {/* bio card */}
        <div style={{ position:"relative", background:"linear-gradient(145deg,rgba(0,35,28,0.6) 0%,rgba(3,13,16,0.88) 100%)", border:"1px solid rgba(0,255,209,0.12)", borderRadius:8, padding: isMobile ? "28px 20px" : "40px 40px 36px" }}>
          ...
          <h2 style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:"clamp(1.3rem,4vw,2rem)", textTransform:"uppercase", color:"#00ffd1", margin:"0 0 24px", textShadow:"0 0 24px rgba(0,255,209,0.2)" }}>A Deeper Look Into My Work</h2>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
            {STATS.map((st, i) => (
              <div key={i} style={{
                ...
                borderRadius:6, padding: isMobile ? "20px 14px" : "28px 22px", textAlign:"center",
              }}>
                ...
                <p style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:900, fontSize:"clamp(1.8rem,5vw,2.8rem)", color:"#00ffd1", margin:"0 0 6px", textShadow:"0 0 24px rgba(0,255,209,0.4)", lineHeight:1 }}>{st.value}</p>
                <p style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700, fontSize:9, letterSpacing:"0.22em", textTransform:"uppercase", color:"rgba(0,255,209,0.45)", margin:0 }}>{st.label}</p>
              </div>
            ))}
          </div>

          {/* quote */}
          <div style={{ position:"relative", padding: isMobile ? "22px 20px" : "28px 28px", background:"linear-gradient(135deg,rgba(0,30,24,0.55) 0%,rgba(3,13,16,0.8) 100%)", border:"1px solid rgba(0,255,209,0.1)", borderRadius:6 }}>
      ...
      {/* ── CORE SKILLS ── */}
      <section style={{ maxWidth:1280, margin: isMobile ? "52px auto 0" : "70px auto 0", padding:`0 ${px}` }}>
        <div style={{ display:"flex", alignItems:"flex-end", justifyContent:"space-between", marginBottom:36, flexWrap:"wrap", gap:16 }}>
          <div>
            ...
            <h2 style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:"clamp(1.6rem,4vw,2.6rem)", textTransform:"uppercase", color:"#fff", margin:0 }}>Core Skills</h2>
          </div>
          {!isMobile && <span style={{ height:1, width:120, background:"linear-gradient(90deg,rgba(0,255,209,0.3),transparent)", display:"block" }} />}
        </div>
      ...
      {/* ── TIMELINE ── */}
      <section style={{ maxWidth:1280, margin: isMobile ? "52px auto 0" : "72px auto 0", padding:`0 ${px}` }}>
        ...
        <h2 style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:"clamp(1.6rem,4vw,2.6rem)", textTransform:"uppercase", color:"#fff", margin:"0 0 40px" }}>My Journey</h2>
        ...
        <div style={{ position:"relative", paddingLeft: isMobile ? 24 : 32 }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:0 }}>
            {TIMELINE.map((item, i) => (
              <div key={i} style={{ position:"relative", paddingBottom: i < TIMELINE.length - 1 ? (isMobile ? 28 : 40) : 0 }}>
                {/* dot */}
                <div style={{ position:"absolute", left: isMobile ? -31 : -40, top:4, width:14, height:14, borderRadius:"50%", border:"1.5px solid #00ffd1", background:"#030d10", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 0 12px rgba(0,255,209,0.4)" }}>
                  <span style={{ width:5, height:5, borderRadius:"50%", background:"#00ffd1", display:"block" }} />
                </div>
                <div style={{ background:"linear-gradient(145deg,rgba(0,28,22,0.55) 0%,rgba(3,13,16,0.8) 100%)", border:"1px solid rgba(0,255,209,0.1)", borderRadius:6, padding: isMobile ? "18px 16px" : "24px 28px" }}>
                  {/* year + title: stack on mobile */}
                  <div style={{ display:"flex", alignItems: isMobile ? "flex-start" : "center", gap: isMobile ? 8 : 14, marginBottom:10, flexDirection: isMobile ? "column" : "row", flexWrap:"wrap" }}>
                    <span style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:13, letterSpacing:"0.2em", color:"#00ffd1", background:"rgba(0,255,209,0.08)", border:"1px solid rgba(0,255,209,0.2)", padding:"3px 10px", borderRadius:3, flexShrink:0 }}>{item.year}</span>
                    <h3 style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize: isMobile ? 16 : 18, textTransform:"uppercase", color:"#e0f5f0", margin:0 }}>{item.title}</h3>
      ...
      {/* ── CTA ── */}
      <section style={{ maxWidth:1280, margin: isMobile ? "52px auto 0" : "70px auto 0", padding:`0 ${px} ${isMobile ? "72px" : "120px"}` }}>
        <div style={{ position:"relative", padding: isMobile ? "36px 20px" : "52px 48px", background:"linear-gradient(135deg,rgba(0,40,32,0.6) 0%,rgba(3,13,16,0.88) 100%)", border:"1px solid rgba(0,255,209,0.12)", borderRadius:8, textAlign:"center", overflow:"hidden" }}>
          ...
          <div style={{ display:"flex", gap:14, justifyContent:"center", flexWrap:"wrap", position:"relative" }}>
            ...
            <a href="/portfolio" style={{ display:"inline-flex", alignItems:"center", justifyContent:"center", gap:8, padding: isMobile ? "13px 24px" : "14px 32px", background:"transparent", border:"1px solid rgba(0,255,209,0.25)", color:"rgba(200,230,225,0.65)", fontFamily:"'Barlow Condensed',sans-serif", fontWeight:700, fontSize:12, letterSpacing:"0.22em", textTransform:"uppercase", borderRadius:3, textDecoration:"none", width: isMobile ? "100%" : "auto" }}>
              View Portfolio
            </a>
          </div>
        </div>
